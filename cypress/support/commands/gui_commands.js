  ...
  cy.get('body').then(($body) => {
    ...
    if ($body.find('.btn-close-error-alert').length > 0) {
      ...
      if (retries > 0) {
        cy.wait(500)
        cy.log(
          "Alert error didn't appear. checking if alert error will appear again"
        )
...
Cypress.Commands.add('validateInvalidCredentials', (username, password) => {
  cy.visit('/login')
...
Cypress.Commands.add('validateInvalidEmail', (username, password) => {
  cy.visit('/login')
